...
function __kronos_ticket --description "Create Golden, Silver, Diamond, and Sapphire Tickets using ticketer.py"
    ...
    if test "$wizard" -eq 1
        ...
        set sid (__kronos_ask "Domain SID (e.g. S-1-5-21...)" "$def_sid"); or return 1
        set -U __KRONOS_CACHE_SID "$sid"

        # 4. Domain FQDN (Mandatory for Kerberos realm resolution)
        ...
        set domain (__kronos_ask "Domain FQDN (e.g. contoso.local)" "$def_domain"); or return 1
        ...
        set user (__kronos_ask "User to impersonate (e.g. Administrator)" "$def_user"); or return 1
        ...
        # 7. User ID (RID) - Relevant for Golden/Diamond/Sapphire
        ...
        if test "$subaction" = "silver"
            ...
            set spn (__kronos_ask "Target SPN (e.g. MSSQLSvc/ms01.contoso.local:1433)" "$def_spn"); or return 1
    ...
    # Final Validation
    if test -z "$domain"; echo "error: domain FQDN is required"; return 1; end
    if test -z "$sid"; echo "error: domain SID is required"; return 1; end
    if test -z "$hash"; echo "error: krbtgt/service hash is required"; return 1; end
    ...
    # Target DC IP is almost always helpful for realm resolution
    if test -n "$TGT_DC_IP"
    ...
    switch "$subaction"
        case silver
            if test -z "$spn"; echo "error: SPN is required"; return 1; end
        ...
        case sapphire
            if test -z "$auth_user"; echo "error: auth-user is required"; return 1; end
            # ticketer.py Sapphire mode strictly requires -aesKey and -password/hashes to be present in the command line
