...
function __kronos_smb --description "Connect to target using smbclient (SMB)"
    ...
    if not command -v smbclient >/dev/null
        echo "error: smbclient not found in PATH" >&2
        return 1
    end
    ...
    if set -q _flag_kerberos
        ...
        # smbclient expects hash in the password field as --pw-nt-hash
        ...
        if test -n "$pass"
            # Pass directly if not using interactive prompt
            # set -a smb_args $pass
            # Better to let it prompt or use an env var if we wanted non-interactive
        end
    end
