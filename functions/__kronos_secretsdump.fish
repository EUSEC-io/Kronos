...
function __kronos_secretsdump --description "Dump AD secrets using secretsdump.py"
    ...
    if not set -q _flag_quiet
        ...
        # Target IP Priority
        set -l def_target "$__KRONOS_CACHE_SECRETSDUMP_TARGET"
        ...
        set -U __KRONOS_CACHE_SECRETSDUMP_TARGET "$target"

        # Domain Name Priority
        set -l def_domain "$__KRONOS_CACHE_SECRETSDUMP_DOMAIN"
        ...
        set -U __KRONOS_CACHE_SECRETSDUMP_DOMAIN "$domain"

        set -l dump_all (__kronos_ask_confirm "Dump ALL account secrets?" y); or return 1
        if test "$dump_all" = "no"
            set -l def_target_user "$__KRONOS_CACHE_SECRETSDUMP_TARGET_USER"
            ...
            set -U __KRONOS_CACHE_SECRETSDUMP_TARGET_USER "$target_user"
        else
            set target_user ""
        end

        if not set -q _flag_kerberos
            set -l def_auth_user "$__KRONOS_CACHE_SECRETSDUMP_AUTH_USER"
            ...
            set -U __KRONOS_CACHE_SECRETSDUMP_AUTH_USER "$auth_user"
            ...
            set -U __KRONOS_CACHE_SECRETSDUMP_AUTH_VAL "$auth_input"
        ...
        if test -z "$target"; set target "$__KRONOS_CACHE_SECRETSDUMP_TARGET"; end
        if test -z "$target"; set target "$TGT"; end
        if test -z "$target"; set target "$TGT_DC_IP"; end
        if test -z "$target"; set target "$TGT_DC"; end
        if test -z "$target"; set target "$TGT_HOSTS[1]"; end

        if test -z "$domain"; set domain "$__KRONOS_CACHE_SECRETSDUMP_DOMAIN"; end
