...
function __kronos_rdp --description "Connect to target using xfreerdp3 (RDP)"
    argparse h/help X/edit-cmd u/username= p/password= H/hash= d/domain= q/quiet w/wizard -- $argv
    or return 1
    ...
    set -l cmd_list $rdp_bin /v:$target /cert:ignore
    if test "$_rdp_fs" -eq 1; set -a cmd_list /f; end
    if test -n "$_rdp_size"; set -a cmd_list /size:$_rdp_size; end
    if test -n "$_rdp_path"; set -a cmd_list /drive:share,$_rdp_path; end
